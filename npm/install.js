// Downloads the prebuilt hipmmcode binary for this platform on npm install.
...
const BIN_NAME = isWindows ? "hipmmcode.exe" : "hipmmcode";
...
function label() {
  ...
  console.error(`hipmmcode: unsupported platform ${process.platform}`);
  process.exit(1);
}

async function main() {
  const url = `https://github.com/${REPO}/releases/latest/download/hipmmcode-${label()}.tar.gz`;
    ...
    if (fs.existsSync(path.join(dir, "default-skills", "MANIFEST.toml"))) {
      ...
      execFileSync(binPath, ["skill", "install-defaults"], {
        ...
        env: {
          ...
          // at an older hipmmcode.
          HIPMMCODE_DEFAULT_SKILLS_DIR: path.join(dir, "default-skills"),
        },
      });
    }
  } catch (e) {
    console.warn(
      `hipmmcode: default skills sync skipped (${e.message || e}); run \`hipmmcode skill install-defaults\` later`
    );
  }

  console.log("hipmmcode: installed — run `hipmmcode model` to configure a channel");
}

main().catch((e) => {
  console.error(`hipmmcode: install failed: ${e.message}`);
  process.exit(1);
});
